        ...
        for (int i = 0; i < jsonfiles.size(); i++) {
            ...
            try {
                ...
                for (int j = 0; j < barcharts.size(); j++) {
                    ...
                    BitmapEncoder.saveBitmapWithDPI(barcharts.get(j), path + "/" + jsonfilename + j, BitmapFormat.PNG,
                            400);
                }
                for (int j = 0; j < piecharts.size(); j++) {
                    // System.out.println(charts.get(j).getSeriesMap());
                    BitmapEncoder.saveBitmapWithDPI(piecharts.get(j),
                            path + "/" + jsonfilename + (j + barcharts.size()), BitmapFormat.PNG, 400);
                }
...
// ByteCodeTool.input("/Users/liangyufei/Desktop/FieldDetectionTool/tool/target/classes/Test.class");
